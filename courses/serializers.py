...
class LevelSerializer(serializers.ModelSerializer):
    ...
    teacher = UsersSerializer(read_only=True, many=False)
    ...
    def update(self, instance, validated_data):
        ...
        for lesson_data in lesson_data:
            ...
            lesson.material_url = lesson_data.get('material_url', lesson.material_url)
            lesson.save()
...
class CourseSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        for level_data in level_data:
            level1 = levels.pop(0)
            level1.title = level_data.get('title', level1.title)
            level1.teacher = level_data.get('teacher', level1.teacher)
            level1.image = level_data.get('image', level1.image)
            level1.save()
        return instance
